...
document.addEventListener("DOMContentLoaded", () => {
  ...
  initializeMobileEnhancements() // Add this line
})
      ...
      <div class="product-detail-info">
        ...
        ${
          ...
          <div class="size-guide-link">
            <button class="btn-secondary" onclick="openSizeGuide()" style="margin-bottom: 1rem;">
        ...
        ${
          product.layers
            ? `
          <div class="product-detail-features">
            <h4>Mattress Layers</h4>
            <ul class="feature-list">
              ${product.layers.map((layer, index) => `<li>${index + 1}. ${layer}</li>`).join("")}
            </ul>
          </div>
        `
            : ""
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  heroButtons.forEach((button) => {
    button.addEventListener("click", function () {
      const targetSection = this.textContent.includes("Shop") ? "products" : "custom"
